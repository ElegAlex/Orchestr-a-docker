...
export { createUserWithLogin } from './createUser';

// Import de la fonction de modification du login
export { updateUserLogin } from './updateUserLogin';

// Import de la fonction de nettoyage auth
export { cleanAuthUser } from './cleanAuthUser';

// Import de la fonction de nettoyage d'utilisateur orphelin
export { cleanupOrphanUser } from './cleanupOrphanUser';

// Import de la fonction de réparation des IDs utilisateurs
export { repairUserIds } from './repairUserIds';
 ...
 */
async function calculateUserWorkloadInternal(data: { userId: string; startDate: string; endDate: string }) {
  ...
  try {
    ...
    const userCapacity = userData?.capacity || { theoretical: 40, available: 35 };
 ...
 */
export const calculateUserWorkload = functions
  .region('europe-west1')
  .runWith({
    memory: '1GB',
    timeoutSeconds: 120,
  ...
  .https.onCall(async (data, context) => {
    ...
    if (!userId || !startDate || !endDate) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'Les paramètres userId, startDate et endDate sont requis.'
      );
    }
    ...
    try {
      ...
      const weeklyHours = userData.workingHours?.weeklyHours || 35;
      const productivityRate = userData.productivity?.rate || 0.8;
      const workingDays = userData.workingHours?.workingDays || ['monday', 'tuesday', 'wednesday', 'thursday', 'friday'];
      ...
      while (currentWeek <= end) {
        ...
        const snapshot = await calculateWeeklyWorkload(
          userId,
          currentWeek,
          weekEnd,
          {
 ...
 */
export const suggestResourceAllocation = functions
  .region('europe-west1')
  .runWith({
    memory: '1GB',
    timeoutSeconds: 120,
  ...
  .https.onCall(async (data, context) => {
    ...
    try {
      ...
      for (const userDoc of usersSnapshot.docs) {
        ...
        if (skillScore === 0) continue; // Pas de compétences compatibles
 ...
 */
export const onAllocationChange = functions
  .region('europe-west1')
  .firestore
  ...
  .onWrite(async (change, context) => {
    ...
    if (allocation?.userId) userIds.add(allocation.userId);
    if (previousAllocation?.userId) userIds.add(previousAllocation.userId);
...
async function calculateWeeklyWorkload(
  userId: string,
  weekStart: Date,
  weekEnd: Date,
  context: {
  ...
  let overloadRisk: 'none' | 'low' | 'medium' | 'high' | 'critical';

  if (loadRatio <= 0.7) overloadRisk = 'none';
  else if (loadRatio <= 0.85) overloadRisk = 'low';
  else if (loadRatio <= 0.95) overloadRisk = 'medium';
  else if (loadRatio <= 1.1) overloadRisk = 'high';
  else overloadRisk = 'critical';
...
function getMatchingSkills(requiredSkills: string[], userSkills: any[]): any[] {
  return requiredSkills.map(required => {
    ...
    return {
      ...
      validated: userSkill?.managerValidated || false
    };
  }).filter(skill => skill.userLevel);
}

function getRecommendation(skillScore: number, availabilityScore: number): string {
...
function generateSuggestions(weeklySnapshots: any[], userData: any): any[] {
...
function calculateOverallRisk(snapshots: any[]): 'none' | 'low' | 'medium' | 'high' | 'critical' {
  ...
  if (riskCounts.high > snapshots.length * 0.3) return 'high';
  if (riskCounts.medium > snapshots.length * 0.5) return 'medium';
