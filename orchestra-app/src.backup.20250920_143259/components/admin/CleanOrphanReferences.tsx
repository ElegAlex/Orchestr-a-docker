...
const CleanOrphanReferences: React.FC = () => {
  ...
  const cleanOrphanReferences = async () => {
    console.log('🧹 [DEBUG] CleanOrphanReferences button clicked!');
    setLoading(true);
    setError(null);
    setResult(null);
    ...
    try {
      ...
      console.log(`🧹 Found ${orphanReferences.length} orphan references across ${projectsToUpdate.length} projects`);
