...
const PlanningCalendar: React.FC<PlanningCalendarProps> = ({
  ...
  const loadCalendarData = useCallback(async () => {
    try {
      ...
      console.log('🔍 USERS DEBUG - Total users from DB:', allUsers.length);
      console.log('🔍 USERS DEBUG - Filtered users:', filteredUsers.length);
      ...
      if (lahbibInAll) {
        console.log('🔍 USERS DEBUG - Lahbib found in allUsers:', {
          id: lahbibInAll.id,
          displayName: lahbibInAll.displayName,
          email: lahbibInAll.email,
          isActive: lahbibInAll.isActive,
          role: lahbibInAll.role
        });
      } else {
        console.log('🔍 USERS DEBUG - Lahbib NOT found in allUsers');
      }

      if (lahbibInFiltered) {
        console.log('🔍 USERS DEBUG - Lahbib found in filteredUsers:', {
          id: lahbibInFiltered.id,
          displayName: lahbibInFiltered.displayName,
          email: lahbibInFiltered.email,
          isActive: lahbibInFiltered.isActive,
          role: lahbibInFiltered.role
        });
      } else {
        console.log('🔍 USERS DEBUG - Lahbib NOT found in filteredUsers (excluded by filter)');
      }
          ...
          try {
            ...
            for (const day of weekDays) {
              ...
              try {
                ...
                console.error(`Erreur télétravail spécifique pour ${user.displayName} le ${dateStr}:`, error);
              }
            }
          } catch (error) {
            ...
            try {
              ...
              const periodTasks = allUserTasks.filter(task => {
                ...
                if (!task.startDate && !task.dueDate) {
                  console.log(`❌ Tâche exclue (aucune date): ${task.title}`, { startDate: task.startDate, dueDate: task.dueDate });
                  return false;
                }
              ...
              if (periodTasks.length > 0) {
                console.log(`✅ Tâches filtrées pour ${user.displayName || user.email} le ${date.toDateString()}:`, periodTasks.length, periodTasks.map(t => t.title));
              }
  ...
  const handleToggleWeeklyRemoteDay = useCallback(async (
    ...
    try {
      ...
      console.error('Erreur lors de la modification du planning hebdomadaire:', error);
    }
  }, []);
  ...
  const handleToggleSpecificRemoteDay = useCallback(async (
    ...
    try {
      ...
      console.error('Erreur lors de la modification du télétravail spécifique:', error);
    }
  ...
  const handleDeleteSpecificRemoteDay = useCallback(async (
    ...
    try {
      ...
      console.error('Erreur lors de la suppression de l\'exception télétravail:', error);
    }
  }, []);
