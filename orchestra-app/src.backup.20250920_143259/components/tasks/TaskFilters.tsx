...
import { TaskStatus, Priority, TaskType, User, Project, Milestone } from '../../types';
...
const TASK_STATUSES = [
  { value: 'TODO', label: 'À faire', color: '#2196f3' },
  { value: 'IN_PROGRESS', label: 'En cours', color: '#ff9800' },
  { value: 'DONE', label: 'Terminé', color: '#4caf50' },
  { value: 'BLOCKED', label: 'Bloqué', color: '#f44336' },
];

const PRIORITIES = [
  { value: 'P0', label: 'P0 - Critique', color: '#f44336' },
  { value: 'P1', label: 'P1 - Élevé', color: '#ff9800' },
  { value: 'P2', label: 'P2 - Normal', color: '#2196f3' },
  { value: 'P3', label: 'P3 - Faible', color: '#9e9e9e' },
];

const TASK_TYPES = [
  { value: 'EPIC', label: '🏔️ Epic', color: '#9c27b0' },
  { value: 'STORY', label: '📝 Story', color: '#2196f3' },
  { value: 'TASK', label: '✅ Task', color: '#4caf50' },
  { value: 'BUG', label: '🐛 Bug', color: '#f44336' },
  { value: 'SPIKE', label: '🔬 Spike', color: '#ff9800' },
];
...
const RISK_LEVELS = [
  ...
  { value: 'critical', label: '🔴 Critique', color: '#f44336' },
];
  ...
  const clearFilter = (filterType: keyof TaskFilterState) => {
    switch (filterType) {
      ...
      case 'assigneeIds':
        ...
        break;
      case 'milestoneIds':
        handleFilterChange('milestoneIds', []);
        break;
      case 'labels':
      ...
      <CardContent>
        ...
        <Collapse in={expanded}>
          <Box sx={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
            ...
            <Box sx={{ flexGrow: 1, minWidth: 200 }}>
              <FormControl fullWidth>
                <InputLabel>Priorités</InputLabel>
                <Select
                  multiple
            ...
            <Box sx={{ flexGrow: 1, minWidth: 200 }}>
              <Autocomplete
                ...
                renderInput={(params) => (
                  <TextField {...params} label="Assignés" />
                )}
            ...
            <Box sx={{ flexGrow: 1, minWidth: 200 }}>
              <Autocomplete
                ...
                value={milestones.filter(milestone => filters.milestoneIds.includes(milestone.id))}
                onChange={(_, value) => handleFilterChange('milestoneIds', value.map(m => m.id))}
                renderInput={(params) => (
                  <TextField {...params} label="Jalons" />
                )}
              />
            </Box>
