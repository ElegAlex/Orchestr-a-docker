...
export const TaskCard: React.FC<TaskCardProps> = ({
  ...
  // NE PAS charger les subtasks ici, trop de calls Firestore
  // On affiche juste un indicateur statique
  const [subtaskCount, setSubtaskCount] = React.useState<{total: number, completed: number}>({total: 0, completed: 0});

  React.useEffect(() => {
    // Charger le count UNE SEULE FOIS au mount
    if (task.id) {
      taskService.getTasksByParentId(task.id).then(subtasks => {
        setSubtaskCount({
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          {subtaskCount.total > 0 && (
            <Chip
              icon={<SubtaskIcon sx={{ fontSize: 12 }} />}
              label={`${subtaskCount.completed}/${subtaskCount.total}`}
              size="small"
              variant="outlined"
              sx={{
                height: 20,
                fontSize: '0.65rem',
                borderColor: subtaskCount.completed === subtaskCount.total ? 'success.main' : 'primary.main',
                color: subtaskCount.completed === subtaskCount.total ? 'success.main' : 'primary.main',
                '& .MuiChip-icon': {
                  color: subtaskCount.completed === subtaskCount.total ? 'success.main' : 'primary.main',
                },
              }}
