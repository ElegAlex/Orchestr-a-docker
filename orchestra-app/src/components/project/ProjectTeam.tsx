...
const ProjectTeam: React.FC<ProjectTeamProps> = ({ project, onRefresh }) => {
        ...
        const members = await Promise.all(
          project.teamMembers.map(async (member) => {
            try {
              ...
              console.log('🔍 [DEBUG] User loaded for team:', user?.displayName, 'Skills:', userSkills);
              return {
                ...user,
  ...
  const handleRemoveMember = async (memberDisplayId: string) => {
    console.log('🔥 [DEBUG] handleRemoveMember called with:', memberDisplayId);
    if (window.confirm('Êtes-vous sûr de vouloir retirer ce membre de l\'équipe ?')) {
      try {
        console.log('🔥 [DEBUG] User confirmed removal, proceeding...');
        console.log('🔥 [DEBUG] Removing member with displayId:', memberDisplayId);
        ...
        console.log('🔥 [DEBUG] Member to remove found:', memberToRemove);

        const userIdToRemove = memberToRemove?.originalUserId || memberDisplayId;
        console.log('🔥 [DEBUG] User ID to remove:', userIdToRemove);
        console.log('🔥 [DEBUG] Current team members:', project.teamMembers);

        const updatedTeamMembers = project.teamMembers?.filter(
          member => member.userId !== userIdToRemove
        );
        console.log('🔥 [DEBUG] Updated team members:', updatedTeamMembers);
        ...
        console.log('🔥 [DEBUG] Clean project to save:', cleanProject);
  ...
  const handleMenuClick = (event: React.MouseEvent<HTMLElement>, memberId: string) => {
    console.log('🔥 [DEBUG] handleMenuClick called with memberId:', memberId);
