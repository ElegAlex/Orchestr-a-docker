...
export const SkillEditor: React.FC<SkillEditorProps> = ({
  ...
  const handleAddSkill = async () => {
    ...
    try {
      ...
      setAvailableSkills(skillsByCategory);
      console.log('✅ Compétence ajoutée et sauvegardée:', newSkillName.trim());
    } catch (error) {
  ...
  const handleAddPredefinedSkill = async (skillName: string, category: SkillCategory) => {
    ...
    try {
      ...
      setSkills([...skills, newSkill]);
      console.log('✅ Compétence prédéfinie ajoutée:', skillName);
    } catch (error) {
      console.error('Erreur lors de l\'ajout de la compétence prédéfinie:', error);
    }
  };
