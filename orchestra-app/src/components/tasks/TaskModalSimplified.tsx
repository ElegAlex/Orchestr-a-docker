...
export const TaskModalSimplified: React.FC<TaskModalProps> = ({
  ...
  const { subtasks, createSubtask, updateSubtask, deleteSubtask, stats, refreshSubtasks } = useSubtasks(task?.id);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Ajouter les dates seulement si définies
      if (formData.startDate) {
        taskData.startDate = new Date(formData.startDate);
      }
      if (formData.dueDate) {
  ...
  return (
    ...
    >
      ...
      <DialogContent sx={{ px: 3, py: 2, height: 'calc(90vh - 200px)', overflowY: 'auto' }}>
        ...
        {/* Section Sous-tâches - seulement pour les tâches existantes */}
        {task && (
          <>
            <Divider sx={{ my: 3 }} />
            <Box sx={{ mb: 3 }}>
              <Typography variant="h6" sx={{ mb: 2, color: 'primary.main', fontWeight: 600 }}>
                🔗 Sous-tâches ({stats.completed}/{stats.total})
              </Typography>
                ...
                {subtasks.map((subtask) => (
                  <Box
                    ...
                    sx={{
                      ...
                      mb: 1,
                      borderRadius: 1,
                      bgcolor: subtask.status === 'DONE' ? 'success.50' : 'grey.50',
                      '&:hover': {
                        bgcolor: subtask.status === 'DONE' ? 'success.100' : 'grey.100',
                      },
                    }}
                  >
                    <IconButton
      ...
      <DialogActions sx={{ px: 3, py: 2 }}>
        ...
        {task && onDelete && (
          <Button 
            color="error" 
            onClick={() => {
              console.log('🗑️ Suppression de la tâche avec ID:', task.id);
              if (task.id && task.id.trim() !== '') {
                onDelete(task.id);
              } else {
                console.error('❌ Impossible de supprimer: ID de tâche manquant ou invalide');
