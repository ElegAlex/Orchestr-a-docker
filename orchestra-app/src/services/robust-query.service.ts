...
export class RobustQueryService {
  ...
  async queryWithRetry(
    ...
    for (let attempt = 1; attempt <= opts.maxRetries; attempt++) {
      try {
        ...
        if (attempt === 2 && opts.autoCleanCache) {
          console.log('🧹 Nettoyage cache automatique...');
          const cleaned = await cacheManagerService.autoRecover();
          if (cleaned) {
            console.log('✅ Cache nettoyé, nouvelle tentative...');
          }
        }
    ...
    console.error('💥 ÉCHEC COMPLET - Toutes les tentatives ont échoué');
  ...
  async getDocumentWithRetry(
    ...
    for (let attempt = 1; attempt <= opts.maxRetries; attempt++) {
      try {
        ...
        lastError = error as Error;
        console.error(`❌ Tentative document ${attempt} échouée:`, error);
        
        // Si c'est la dernière tentative, pas de retry
        if (attempt === opts.maxRetries) {
          console.error(`💥 Toutes les tentatives document ont échoué`);
          break;
        }
        
        // Nettoyage automatique du cache à la 2ème tentative
        if (attempt === 2 && opts.autoCleanCache) {
          console.log('🧹 Nettoyage cache automatique pour document...');
          const cleaned = await cacheManagerService.autoRecover();
          if (cleaned) {
            console.log('✅ Cache nettoyé pour document, nouvelle tentative...');
          }
        }
    ...
    // Toutes les tentatives ont échoué
    console.error('💥 ÉCHEC COMPLET DOCUMENT - Toutes les tentatives ont échoué');
  ...
  async queryWithFallback(query: Query): Promise<QuerySnapshot> {
    try {
      ...
      try {
        ...
        console.error('💥 Serveur aussi échoué:', serverError);
  ...
  async batchQueries<T>(
    ...
    const promises = queries.map(async ({ name, query, transform }) => {
      try {
        ...
        console.log(`✅ Batch ${name}: succès`);
      } catch (error) {
        console.error(`❌ Batch ${name}: échec`, error);
  ...
  async testConnectivity(): Promise<{
    ...
    try {
      ...
      try {
        ...
        console.log('❌ Serveur défaillant');
      }
