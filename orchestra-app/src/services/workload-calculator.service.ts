...
class WorkloadCalculatorService {
  ...
  async calculateUserWorkload(
    ...
    try {
      ...
      console.log(`✅ Workload calculé: ${plannedWorkload.totalEstimatedHours}h planifiées / ${capacity.availableDays * 7}h disponibles`);
      
      return workloadCalculation;
      
    } catch (error) {
      console.error(`❌ Erreur calcul workload pour ${userId}:`, error);
      throw error;
    }
  }
  ...
  async calculateTeamWorkload(
    ...
    const promises = userIds.map(async (userId) => {
      try {
        ...
        console.error(`Erreur calcul pour ${userId}:`, error);
        return null;
      }
    });
    ...
    console.log(`✅ Workload équipe calculé: ${Object.keys(calculations).length} utilisateurs`);
